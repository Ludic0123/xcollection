...
alter table public.members
  add column if not exists best_restaurant_1 text,
  add column if not exists best_restaurant_2 text,
  add column if not exists best_restaurant_3 text;

-- handle_new_member を再定義（v15 + best_restaurant_1..3）
...
begin
  ...
  insert into public.members (
    ...
    best_restaurant_1, best_restaurant_2, best_restaurant_3
  ) values (
    ...
    nullif(m->>'best_restaurant_1', ''),
    nullif(m->>'best_restaurant_2', ''),
    nullif(m->>'best_restaurant_3', '')
  );
