...
export default async function TripDetailPage({
  ...
  const { data: planData } = await supabase
    ...
    .eq('id', id)
    .single()
  if (!planData || isTestContent(planData.title)) notFound()
    ...
    <div className="bg-white min-h-screen">
      ...
      <section className="px-8 md:px-16 py-12 grid grid-cols-1 md:grid-cols-12 gap-10 border-b hairline">
        <div className="md:col-span-4 space-y-6">
          <div>
            <p className="text-xs tracking-luxe text-neutral-600">TOTAL BUDGET</p>
            <p className="font-serif text-5xl mt-2">¥{totalBudget.toLocaleString()}</p>
          </div>
          <div className="flex gap-8">
            <div>
              <p className="text-xs tracking-luxe text-neutral-600">ITEMS</p>
              <p className="font-serif text-2xl mt-1">{items.length}</p>
            </div>
            <div>
              <p className="text-xs tracking-luxe text-neutral-600">DAYS</p>
      ...
      <section className="px-8 md:px-16 py-12">
        <div className="flex items-baseline justify-between mb-8">
          <div>
            <p className="text-xs tracking-luxe text-neutral-600">ITINERARY</p>
          ...
          <div className="space-y-12">
            {days.map((d) => (
              <div key={d}>
                ...
                {(byDay[d] ?? []).length === 0 ? (
                  <p className="text-xs text-neutral-600 py-4">この日には予定がありません</p>
                ) : (
