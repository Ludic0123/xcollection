...
export default async function HotelsListPage({
  ...
  let query = supabase.from('hotels').select('*').not('name', 'like', TEST_CONTENT_PATTERN).order('created_at', { ascending: false })
  ...
  return (
    <div className="bg-white min-h-screen">
      <div className="px-4 md:px-16 pt-4 pb-3 md:pt-12 md:pb-8 border-b hairline">
        ...
        <p className="text-xs md:text-sm text-neutral-500 mt-1 md:mt-2">{list.length} hotels</p>
      </div>

      <form className="px-4 md:px-16 py-2.5 md:py-5 border-b hairline flex flex-wrap gap-3 md:gap-4 items-end">
        <div>
          <p className="text-xs tracking-luxe text-neutral-600 mb-0.5">SEARCH</p>
          <input
            name="q"
            defaultValue={params.q ?? ''}
            placeholder="ホテル名"
            className="min-h-11 max-w-full border-b hairline bg-transparent px-1 py-2 text-xs md:text-sm w-32 md:w-48 focus:outline-none focus:border-black"
          />
        </div>
        <div>
          <p className="text-xs tracking-luxe text-neutral-600 mb-0.5">PREFECTURE</p>
          <select
            name="prefecture"
            defaultValue={params.prefecture ?? ''}
            className="min-h-11 max-w-full border-b hairline bg-transparent px-1 py-2 text-xs md:text-sm focus:outline-none focus:border-black"
          >
        ...
        <div>
          <p className="text-xs tracking-luxe text-neutral-600 mb-0.5">BRAND</p>
          <select
            name="brand"
        ...
        <button className="ui-action text-xs tracking-luxe border-b border-black px-1 py-0.5 hover:opacity-50">
          FILTER
        </button>
        <Link href="/hotels" className="ui-action text-xs tracking-luxe text-neutral-600 hover:text-black">
          RESET
        </Link>
      </form>
      ...
      <div className="px-4 md:px-16 py-6">
        {list.length === 0 ? (
          <div className="py-24 text-center text-sm text-neutral-600">まだ登録がありません。</div>
        ) : (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-3 mt-1 text-xs text-neutral-500 flex-wrap">
                      {h.price_range && (
                        <span className="text-neutral-600">{'¥'.repeat(h.price_range)}</span>
                      )}
                      {h.want_to_visit && (
