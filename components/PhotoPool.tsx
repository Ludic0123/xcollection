...
export type PoolPhoto = { url: string; caption: string; ingredients: string[] }
...
export default function PhotoPool({
  ...
  value: PoolPhoto[]
  onChange: React.Dispatch<React.SetStateAction<PoolPhoto[]>>
  folder?: string
  max?: number
  ...
  async function handleFiles(files: FileList) {
    ...
    async function uploadOne(file: File): Promise<{ url: string | null; error: string | null }> {
      try {
        ...
        return { url: null, error: e instanceof Error ? e.message : 'アップロード失敗' }
      }
    }
    ...
    for (let i = 0; i < toUpload.length; i += concurrency) {
      ...
      if (newPhotos.length > 0) {
        onChange(current => [...current, ...newPhotos])
      }
    }
    const failed = allResults.filter((r) => !r.url)
    if (failed.length > 0) setError(`${failed.length}枚のアップロードに失敗: ${failed[0].error ?? ''}`)
    } catch (error) {
  ...
  function setCaption(idx: number, caption: string) {
    onChange(current => current.map((p, i) => (i === idx ? { ...p, caption } : p)))
  }
  function toggleIngredient(idx: number, name: string) {
    onChange(current =>
      current.map((p, i) => {
        ...
        return {
          ...p,
          ingredients: has ? p.ingredients.filter((n) => n !== name) : [...p.ingredients, name],
        }
      })
    )
  }
  ...
  return (
    <div>
      <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
        ...
        {value.length < max && (
          ...
          >
            ...
            <span className="text-[10px] text-neutral-500 mt-1">{uploading ? '送信中…' : '追加'}</span>
            <input
              type="file"
              accept="image/*"
              multiple
      ...
      <p className="text-[10px] text-neutral-400 mt-2">
        {value.length} / {max} 枚 ・ まとめて選択できます。各写真に名前・食材をつけられます
      </p>
